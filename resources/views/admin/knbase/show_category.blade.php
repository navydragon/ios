    ...
    <div class="col-md-12 col-xl-4">
        <div class="card mb-4">
            ...
            <div class="dropdown"  id="nesting-dropdown-demo">
                <button type="button" class="btn btn-default  dropdown-toggle" data-toggle="dropdown" aria-expanded="false">{{$category->name}}</button>
                <div class="dropdown-menu" style="">
                    <a class="dropdown-item" href="/adm/kn_base/">Без категории</a>
                    @foreach ($categories as $cat)
                        <a class="dropdown-item" href="/adm/kn_base/{{$cat->id}}">{{$cat->name}}</a>
                    @endforeach
                </div>
            </div>
            </div>
...
<div class="row">
    <div class="col-md-12 col-xl-12">
        <div class="card mb-4">
            <h4 class="m-4">Материалы категории «{{$category->name}}»</h4>
			<div class="ml-4 pull-right">
				<form method="POST" id="category_form" action="/adm/kn_base/{{$category->id}}">
					@csrf
					{{ method_field('DELETE') }}
					...
					<button type="button" onclick="delete_category()" class="btn btn-sm btn-danger"><i class="fa fa-times"></i> Удалить категорию</button>
				</form>
			</div>
	...
	<div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
      	...
      	<form method="POST" enctype="multipart/form-data" action="/adm/knbase/{{$category->id}}/add_material">
			@csrf
        	<div class="modal-body">
				<div class="row">
					<div class="col-md-12">
